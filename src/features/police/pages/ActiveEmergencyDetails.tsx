  ...
  return (
    <AppShell userRole="POLICE" connectionState="connected">
      <div className="flex flex-col h-full bg-bg-main overflow-hidden">
        ...
        {/* Header */}
        <div className="bg-bg-surface border-b border-border-subtle px-4 py-3 flex items-center justify-between shrink-0">
          <div className="flex items-center gap-3">
            <button
              onClick={() => navigate('/police')}
              className="p-1.5 rounded-lg bg-bg-elevated text-text-secondary hover:text-text-primary transition-colors cursor-pointer"
            >
        ...
        {/* Map Area */}
        <div className="flex-1 relative min-h-0 border-b border-border-subtle">
        ...
        <div className="bg-bg-surface p-4 shrink-0 space-y-3 max-h-[45vh] overflow-y-auto">
          ...
          <div className="space-y-1.5">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
              {junctions.slice(0, 3).map(j => (
                <div key={j.id} className="bg-bg-main p-2 rounded-lg border border-border-subtle flex items-center justify-between text-xs">
                  <div className="min-w-0 pr-2">
                    <p className="font-bold text-text-primary truncate">{j.name}</p>
                    <p className="text-[10px] text-text-secondary">Dist: {j.distanceFromAmbulanceMeters || 850}m</p>
                  </div>
                  <div className="flex gap-1 shrink-0">
                    <button
                      onClick={() => handleJunctionClearance(j.id, 'CLEARED')}
                      className={`px-2 py-0.5 rounded text-[10px] font-bold ${
                        j.status === 'CLEARED' ? 'bg-emerald-600 text-white' : 'bg-bg-surface text-text-secondary hover:bg-emerald-950 hover:text-emerald-300'
                      }`}
                    >
                      Clear
                    </button>
                    <button
                      ...
                      className={`px-2 py-0.5 rounded text-[10px] font-bold ${
                        j.status === 'PASSED' ? 'bg-sky-600 text-white' : 'bg-bg-surface text-text-secondary hover:bg-sky-950 hover:text-sky-300'
                      }`}
                    >
                      Passed
                    </button>
