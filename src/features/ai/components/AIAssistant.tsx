  ...
  return (
    <>
      ...
      <button
        ...
        className={`fixed bottom-6 right-6 z-[999] w-14 h-14 bg-bg-elevated hover:bg-cyan-900 border border-cyan-500/50 text-cyan-400 rounded-full shadow-[0_0_20px_rgba(8,145,178,0.3)] flex items-center justify-center transition-all hover:scale-105 ${isOpen ? 'scale-0 opacity-0 pointer-events-none' : 'scale-100 opacity-100'}`}
      >
      ...
      {/* Main Chat Panel */}
      <div
        className={`fixed bottom-6 right-6 sm:bottom-8 sm:right-8 z-[1000] w-full sm:w-[420px] max-w-[calc(100vw-32px)] h-[calc(100vh-64px)] sm:h-[650px] max-h-[800px] bg-bg-main/95 backdrop-blur-md border border-cyan-800/40 rounded-2xl shadow-2xl flex flex-col overflow-hidden transition-all duration-300 origin-bottom-right ${isOpen ? 'scale-100 opacity-100' : 'scale-95 opacity-0 pointer-events-none'}`}
      >
        {/* Header */}
        <div className="bg-bg-surface/80 p-3.5 border-b border-cyan-900/40 flex items-center justify-between shrink-0">
          <div className="flex items-center gap-3">
            <button 
              onClick={() => setShowHistory(!showHistory)}
              className="p-1.5 text-text-secondary hover:text-cyan-400 hover:bg-bg-elevated rounded transition-colors"
            ...
            <div className="w-8 h-8 rounded-full bg-cyan-900/40 border border-cyan-500/40 flex items-center justify-center relative shadow-[0_0_10px_rgba(8,145,178,0.2)]">
              ...
              <span className="absolute -bottom-0.5 -right-0.5 w-2.5 h-2.5 bg-emerald-500 border-2 border-bg-surface rounded-full"></span>
            </div>
            <div>
              <h3 className="font-bold text-text-primary text-[13px] tracking-wide">AERO INTELLIGENCE</h3>
          ...
          <div className="flex items-center gap-1">
            <button onClick={createNewChat} className="p-1.5 text-text-secondary hover:text-cyan-400 transition-colors" title="New Chat">
              <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><line x1="12" y1="5" x2="12" y2="19"></line><line x1="5" y1="12" x2="19" y2="12"></line></svg>
            </button>
            <button onClick={() => setIsOpen(false)} className="p-1.5 text-text-secondary hover:text-red-400 transition-colors" title="Close">
        ...
        <div className="flex-1 relative overflow-hidden flex flex-col">
          {/* History Sidebar */}
          <div className={`absolute inset-y-0 left-0 w-64 bg-bg-surface/95 border-r border-border-subtle z-10 transition-transform duration-300 overflow-y-auto ${showHistory ? 'translate-x-0' : '-translate-x-full'}`}>
            <div className="p-4">
              <h4 className="text-xs font-bold text-text-secondary uppercase tracking-wider mb-4">Chat History</h4>
              {conversations.length === 0 ? (
                <p className="text-sm text-text-primary0 text-center py-4">No recent conversations.</p>
              ) : (
                <div className="space-y-1">
                  {conversations.map(conv => (
                    <div 
                      ...
                      className={`group flex items-center justify-between p-2.5 rounded-lg cursor-pointer transition-colors ${activeConversationId === conv.id ? 'bg-cyan-900/30 border border-cyan-800/50' : 'hover:bg-bg-elevated'}`}
                    >
                      <div className="truncate flex-1 min-w-0">
                        <p className="text-sm text-text-primary truncate font-medium">{conv.title}</p>
                        <p className="text-[10px] text-text-primary0">{new Date(conv.created_at).toLocaleDateString()}</p>
                      </div>
                      <button 
                        onClick={(e) => deleteConversation(e, conv.id)}
                        className="opacity-0 group-hover:opacity-100 p-1.5 text-text-primary0 hover:text-red-400 transition-all shrink-0"
                      >
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-5" onClick={() => setShowHistory(false)}>
            ...
            {isLoading && (
              <div className="flex flex-col items-start">
                ...
                <div className="bg-bg-elevated border border-border-subtle rounded-2xl rounded-tl-sm p-4 flex items-center gap-2">
                  <span className="text-xs text-text-secondary mr-2 font-medium">Analyzing context...</span>
        ...
        {/* Input Area */}
        <div className="p-3 bg-bg-surface/80 border-t border-cyan-900/40 shrink-0">
          <div className="relative flex items-end bg-bg-main border border-border-subtle rounded-xl focus-within:border-cyan-600 focus-within:ring-1 focus-within:ring-cyan-600/50 transition-all">
            <textarea
              value={input}
          ...
          <div className="flex justify-between items-center mt-2 px-1">
            <span className="text-[9px] text-text-primary0 font-medium">ENTER to send, SHIFT+ENTER for new line</span>
          </div>
        </div>
      </div>
    </>
